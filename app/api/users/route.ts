...
export async function POST(request:Request){const b=await requireAdmin();if(b)return b;const p=input.safeParse(await request.json());if(!p.success)return NextResponse.json({error:'Invalid user'},{status:400});const d=p.data;const passwordHash=hash(`${randomBytes(16).toString('hex')}:${d.password}`);const user=await query<{id:string}>('INSERT INTO users(email,name,password_hash) VALUES($1,$2,$3) RETURNING id',[d.email,d.name,passwordHash]);const role=await query<{id:string}>('SELECT id FROM roles WHERE name=$1',[d.role]);if(role.rows[0])await query('INSERT INTO user_roles(user_id,role_id) VALUES($1,$2)',[user.rows[0].id,role.rows[0].id]);await log('created','user',user.rows[0].id);return NextResponse.json(user.rows[0],{status:201})}
